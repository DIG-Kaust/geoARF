...
class VelModel:
    ...
    def __init__(self, maxZ: int, maxL: int, minVs: int, maxVs: int, sizex: int, path=str, ranvar=False, minran1=1, maxran1=10, maxranf=20, get_plot=False, maxfreq=40, save_data=False, random_range=50, counter=1, dx=0.5):
            ...
            while True:
                ...
                if remThi[i] <= 0:
                    ...
                    if self.ranvar:
                        ...
                        modran = (50 + 140*(self.Thi[i] / 2 + thi)/50)
    ...
    def get_dispercurves(self, plot=False):
        ...
        # dVelocity_model[n] = Velocity_model use later for saving the models to do the fdelmoc step
    ...
    def save_data(self):
        ...
        np.savetxt(self.path + '/training/model/' + str(self.counter) +
                   '_velocity_model.txt', self.velocity_model, fmt='%10.5f')
        np.save(self.path + '/training/label/' +
